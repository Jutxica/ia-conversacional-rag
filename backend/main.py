...
async def chat_response_generator_anthropic(query: str, scope: str = "Geral", history: list = None, conversation_id: str = None, categories: list = None):
    ...
    if anthropic_client is None:
        ...
        if trace:
            try:
                trace.update(output=error_msg)
                if hasattr(trace, 'end'):
                    trace.end()
            except Exception as e:
                print(f"[LANGFUSE] Erro ao finalizar trace: {e}")
        if langfuse:
            try:
                langfuse.flush()
            except Exception as e:
    ...
    try:
        ...
        # 4. Executar busca híbrida local na base de dados Oracle com instrumentação
        ...
        if trace:
            try:
                ...
                print(f"[LANGFUSE] Erro ao criar span de retrieval: {e}")
        ...
        try:
                ...
                try:
                    retrieval_span.end(
                        output={
                            ...
                            "chunks_count": len(citations_raw),
                            "chunks": chunks_log
                        },
                        metadata={
                            "citations": chunks_log
                        }
                    )
                except Exception as end_e:
                    print(f"[LANGFUSE] Erro ao finalizar span de retrieval: {end_e}")
            ...
            if retrieval_span:
                try:
                    retrieval_span.end(
                        level="ERROR",
                        status_message=str(search_err)
                    )
                except Exception as end_e:
                    print(f"[LANGFUSE] Erro ao finalizar span de retrieval com erro: {end_e}")
        ...
        default_system_prompt = (
            "Você é o \"Dehon AI\", um assistente de inteligência artificial acadêmico e internacional especializado na teologia, biografia, espiritualidade e obra do Venerável Padre João Leão Dehon. "
            "Sua tarefa é fornecer respostas profundas, detalhadas, analíticas e exaustivas para pesquisadores e religiosos dehonianos de todo o mundo. Mantenha um tom científico, teológico, respeitoso, pastoral e elegante, semelhante ao de um ensaio acadêmico de alta qualidade (estilo NotebookLM).\n\n"
            ...
            "2. ANCORAGEM E FIDELIDADE ESTRITA: Baseie as suas respostas exclusivamente nos trechos de documentos fornecidos. Nunca use ou invente conhecimentos externos aos documentos.\n"
            "3. LACUNAS DOCUMENTAIS: Se a informação não estiver disponível nas fontes recuperadas, responda EXATAMENTE com a frase: \"Não encontrei informações sobre isso nos documentos disponíveis.\" Nunca invente informações, fatos, datas ou nomes.\n"
            "4. PROFUNDIDADE ACADÊMICA: Construa uma resposta completa e aprofundada, com uma introdução conceitual, um desenvolvimento minucioso dos fundamentos teológicos/históricos e uma síntese final. Evite respostas genéricas, rasas ou resumidas demais.\n"
            "5. ESTRUTURA E FORMATAÇÃO PREMIUM: Divida a resposta em seções lógicas usando títulos markdown de nível 3 (###), utilize listas estruturadas com marcadores (-) para detalhar pontos explicativos e destaque conceitos teológicos centrais ou nomes em **negrito** (ex: **oblação**, **reparação**, **Ecce Venio**, **redamatio**, **reinado social**). Use *itálico* para títulos de documentos ou obras.\n"
            "6. CITAÇÕES PRECISAS COM [N]: Sempre cite a fonte correspondente usando a numeração exata do identificador da fonte (ex: [1], [2], [3], [4]) exatamente no ponto da afirmação. Nunca faça citações genéricas ou fora do padrão. Não crie uma seção final separada para referências.\n"
            "7. NUNCA utilize colchetes de controle interno ou seções artificiais na sua resposta final (como [Abertura], [Conteúdo], [Fechamento], [Referências] ou [Lacunas Documentais]).\n"
            "8. CITAÇÃO LITERAL vs. PARÁFRASE: Só use aspas (\"...\") quando o texto for reproduzido PALAVRA POR PALAVRA a partir de um trecho recuperado. Nunca coloque entre aspas um texto que você resumiu, adaptou, traduziu ou reformulou. Ao interpretar, analisar ou resumir, use suas próprias palavras SEM aspas e sinalize que é análise (ex.: \"o texto sugere que...\", \"segundo a análise de *Studia Dehoniana* [N]...\"). Nunca apresente uma interpretação como se fosse transcrição literal de Dehon.\n"
            "9. PEDIDOS DE CITAÇÃO EXATA: Quando o pesquisador pedir a \"citação exata\" ou as \"palavras do próprio Dehon\", localize o trecho literal nos documentos recuperados e transcreva-o entre aspas, seguido de [N]. Se o trecho literal não estiver presente nos documentos recuperados, responda \"Não encontrei informações sobre isso nos documentos disponíveis.\" — NÃO construa uma citação a partir de uma paráfrase ou interpretação.\n"
            "10. FONTE PRIMÁRIA vs. SECUNDÁRIA: Trate como FONTE PRIMÁRIA os escritos do próprio Padre Dehon (ex.: NHV, NQ/Notes Quotidiennes, Diário, OSp/Oeuvres Spirituelles, cartas, discursos) e como FONTE SECUNDÁRIA os estudos e comentários sobre Dehon (ex.: *Studia Dehoniana* e demais comentadores). Prefira fontes primárias para fundamentar afirmações e use as secundárias para contextualizar. Deixe claro ao leitor quando uma afirmação vem de Dehon e quando vem de um estudo sobre ele; nunca apresente uma paráfrase de comentador como citação literal de Dehon.\n"
            "11. MENSAGENS CONVERSACIONAIS (EXCEÇÃO ÀS REGRAS 2 e 3): Quando a mensagem do usuário for uma saudação, agradecimento, despedida, conversa social breve (small talk) ou uma pergunta sobre você mesmo (quem você é, o que faz, como usar), NÃO exija documentos e NÃO responda \"Não encontrei informações sobre isso nos documentos disponíveis.\" Nesses casos, responda de forma breve, cordial e natural, no mesmo idioma do usuário, sem títulos markdown, sem listas e sem citações [N], e convide o pesquisador a fazer uma pergunta sobre o Padre Dehon. Aplique as regras 4 a 10 (profundidade acadêmica, estrutura, citações) apenas às perguntas de pesquisa que exigem os documentos recuperados."
        )
        
        prompt_label = get_env_clean("LANGFUSE_PROMPT_LABEL", "production")
        ...
        if langfuse:
            try:
                ...
                history_parts = []
                for h in (history or []):
                    role = "Pesquisador" if h.get("role") == "user" else "Dehon AI"
                ...
                user_prompt = query
                print(f"[LANGFUSE] Prompt 'Central' carregado e compilado com sucesso para Claude (versão: {getattr(langfuse_prompt, 'version', 'desconhecida')})")
            except Exception as pe:
                print(f"[LANGFUSE] Erro ao obter/compilar prompt 'Central' para Claude: {pe}. Usando fallback local.")
                langfuse_prompt = None
...
"""
        ...
        if trace:
            try:
                trace.update(output=full_response_content)
                if hasattr(trace, 'end'):
                    trace.end()
            except Exception as e:
                print(f"[LANGFUSE] Erro ao finalizar/encerrar trace: {e}")
        if langfuse:
            try:
                langfuse.flush()
            except Exception as e:
...
async def chat_response_generator_google(query: str, scope: str = "Geral", history: list = None, conversation_id: str = None, categories: list = None):
    ...
    try:
        ...
        default_system_prompt = (
            ...
            "3. LACUNAS DOCUMENTAIS: Se a informação não estiver disponível nas fontes recuperadas, responda EXATAMENTE com a frase: \"Não encontrei informações sobre isso nos documentos disponíveis.\" Nunca invente informações, fatos, datas ou nomes.\n"
            "4. PROFUNDIDADE ACADÊMICA: Construa uma resposta completa e aprofundada, com uma introdução conceitual, um desenvolvimento minucioso dos fundamentos teológicos/históricos e uma síntese final. Evite respostas genéricas, rasas ou resumidas demais.\n"
            "5. ESTRUTURA E FORMATAÇÃO PREMIUM: Divida a resposta em seções lógicas usando títulos markdown de nível 3 (###), utilize listas estruturadas com marcadores (-) para detalhar pontos explicativos e destaque conceitos teológicos centrrais ou nomes em **negrito** (ex: **oblação**, **reparação**, **Ecce Venio**, **redamatio**, **reinado social**). Use *itálico* para títulos de documentos ou obras.\n"
            "6. CITAÇÕES PRECISAS COM [N]: Sempre cite a fonte correspondente usando a numeração exata do identificador da fonte (ex: [1], [2], [3], [4]) exatamente no ponto da afirmação. Nunca faça citações genéricas ou fora do padrão. Não crie uma seção final separada para referências.\n"
            "7. NUNCA utilize colchetes de controle interno ou seções artificiais na sua resposta final (como [Abertura], [Conteúdo], [Fechamento], [Referências] ou [Lacunas Documentais]).\n"
            "8. CITAÇÃO LITERAL vs. PARÁFRASE: Só use aspas (\"...\") quando o texto for reproduzido PALAVRA POR PALAVRA a partir de um trecho recuperado. Nunca coloque entre aspas um texto que você resumiu, adaptou, traduziu ou reformulou. Ao interpretar, analisar ou resumir, use suas próprias palavras SEM aspas e sinalize que é análise (ex.: \"o texto sugere que...\", \"segundo a análise de *Studia Dehoniana* [N]...\"). Nunca apresente uma interpretação como se fosse transcrição literal de Dehon.\n"
            "9. PEDIDOS DE CITAÇÃO EXATA: Quando o pesquisador pedir a \"citação exata\" ou as \"palavras do próprio Dehon\", localize o trecho literal nos documentos recuperados e transcreva-o entre aspas, seguido de [N]. Se o trecho literal não estiver presente nos documentos recuperados, responda \"Não encontrei informações sobre isso nos documentos disponíveis.\" — NÃO construa uma citação a partir de uma paráfrase ou interpretação.\n"
        ...
        if langfuse:
            try:
                ...
                user_prompt = query
                print(f"[LANGFUSE] Prompt 'Central' carregado e compilado com sucesso (versão: {getattr(langfuse_prompt, 'version', 'desconhecida')})")
            except Exception as pe:
                print(f"[LANGFUSE] Erro ao obter/compilar prompt 'Central': {pe}. Usando fallback local.")
                langfuse_prompt = None
