...
- **Testes Automatizados:** Suíte do `pytest` estendida para **30 testes unitários** com 100% de sucesso, cobrindo intent detector, processor de conceitos, normalização, fatiamento Parent-Child robusto e bypass de Record Manager.

---

## 📈 Próximos Passos Imediatos
1.  **Suporte a Coleções na UI:** Adicionar checkboxes no chat para o usuário restringir buscas a escopos específicos (ex: apenas Cartas ou apenas Diários).
2.  **Reranking Híbrido Local:** Explorar compressão de pesos do cross-encoder para execução inteiramente local caso a latência aumente.
3.  **Visualizador de Embeddings 3D:** Desenvolver representação geométrica dos chunks.
